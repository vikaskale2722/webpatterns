...
def run_clustering(req: ClusteringRequest):
    ...
    feat_path = features_dir / f"{base}_features.csv"
    meta_path = features_dir / f"{base}_meta.csv"
    if not feat_path.exists() or not meta_path.exists():
        return {"status": "error", "message": f"Required input files missing for {base}"}
    ...
    if req.algorithm == "kmeans":
        ...
        labels = model.fit_predict(X)
        algo_suffix = f"dbscan_eps{req.eps}_min{req.min_samples}"
    else:
